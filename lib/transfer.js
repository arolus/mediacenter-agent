...
function watchTransfers(ctx) {
  ...
  const client = new WebTorrent(torrentPort ? { torrentPort } : {});
  ...
  function startSeed(id, t) {
    ...
    client.seed(t.filePath, { announce: TRACKERS }, (torrent) => {
  ...
  function startDownload(id, t) {
    ...
    const torrent = client.add(t.magnet, { path: config.mediaDir, announce: TRACKERS });
...

    {
      ...
      torrent.on("done", async () => {
        ...
        const destPath = path.join(config.mediaDir, file.path);
        console.log("transfer: принято", destPath);
        try {
          await addLibraryFile(ctx, destPath, {
            fileName: path.basename(file.path),
            catalogId: t.catalogId, title: t.title, year: t.year, poster: t.poster
          });
        } catch (e) {
