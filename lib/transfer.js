...
function watchTransfers(ctx) {
  ...
  const unsub = onSnapshot(collection(db, "transfers"), (snap) => {
    snap.docChanges().forEach((ch) => {
      ...
      // Роль источника. "seeding"/"downloading" — возобновление после перезапуска агента:
      // приёмник ещё качает, а раздавать уже некому, и перенос молча застывает.
      if (t.source === myId && ["requested", "seeding", "downloading"].includes(t.status) && !seeding.has(id)) {
        startSeed(id, t);
      }
      // Роль приёмника
  ...
  function startSeed(id, t) {
    ...
    client.seed(t.filePath, { announce: [] }, (torrent) => {
      ...
      // Адрес для прямого подключения приёмника. Порт берём фактический: конфиг с
      // torrentPort есть не у всех нод (старые провижены), а без адреса приёмнику
      // остаётся только LSD — и перенос молча стоит на нуле.
      const actualPort = torrentPort || client.torrentPort || (client._server && client._server.address && client._server.address().port);
      const seederAddr = actualPort ? `${lanIp()}:${actualPort}` : null;
      updateDoc(tRef(id), {
