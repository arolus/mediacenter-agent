...
// Установлен ли пакет. Основной путь — Android `pm`, но на Android TV (Xiaomi TV Box S)
// он падает с SecurityException, как и `settings` на телефоне: CLI живёт только под shell-uid.
// Фолбэк — по каталогу данных пакета: он есть у любого установленного приложения, и хотя
// заглянуть внутрь чужого нам не дают, сам факт существования отличим по КОДУ ошибки —
// EACCES у установленного против ENOENT у отсутствующего.
function installedByDataDir(pkg) {
  for (const base of ["/data/data/", "/data/user/0/"]) {
    try {
      fs.statSync(base + pkg);
      return true;                              // доступ есть (редко, но бывает)
    } catch (e) {
      if (e.code === "EACCES" || e.code === "EPERM") return true;
      // ENOENT — идём к следующему пути
    }
  }
  return false;
}
...
function isInstalled(pkg, cb) {
  const name = pkg || DEFAULT_PKG;
  execFile("pm", ["list", "packages", name], { timeout: 8000 }, (err, out) => {
    if (!err) return cb(String(out || "").includes(name));
    cb(installedByDataDir(name));
  });
}
