...
// Техданные файла для детальной страницы TV (кодек/разрешение/звук/длительность) —
// как техчипы у Kodi. Разовый ffprobe, результат кэшируется вызывающей стороной.
function probeMediaInfo(filePath, cb) {
  execFile("ffprobe", [
    "-v", "error", "-show_entries",
    "stream=codec_type,codec_name,width,height,channels,display_aspect_ratio:format=duration",
    "-of", "json", filePath
    ...
    try {
      ...
      cb({
        duration: Math.round(parseFloat(j.format && j.format.duration) || 0),
        vcodec: (v && v.codec_name) || null,
        width: (v && v.width) || 0,
        height: (v && v.height) || 0,
        aspect: (v && v.display_aspect_ratio) || null,
...
// Кадр из середины видео (превью для нераспознанных серий). Пишет jpg в outPath.
function makeThumb(filePath, outPath, cb) {
  execFile("ffprobe", ["-v", "error", "-show_entries", "format=duration", "-of", "csv=p=0", filePath],
    { timeout: 15000 }, (err, out) => {
      const dur = err ? 0 : parseFloat(out) || 0;
      const ss = dur > 2 ? Math.floor(dur / 2) : 0;
      execFile("ffmpeg", ["-hide_banner", "-loglevel", "error",
        "-ss", String(ss), "-i", filePath,
        "-frames:v", "1", "-vf", "scale=480:-2", "-q:v", "5", "-y", outPath
      ], { timeout: 60000 }, (e2) => cb(!e2));
    });
}

module.exports = { ffmpegAvailable, streamTranscode, makeThumb, probeMediaInfo };
