...
const MAX_ACTIVE = 2; // телефон — не кодировочная ферма
let active = 0;
...
function streamTranscode(item, startSec, res) {
  ...
  probeCodecs(item.filePath, (codecs) => {
    ...
    console.log(`transcode: ${item.title || item.fileName} (v:${vCopy ? "copy" : "x264"} a:${aCopy ? "copy" : "aac"}${startSec ? " от " + startSec + "с" : ""})`);
