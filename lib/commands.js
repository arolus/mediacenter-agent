...
// Куда файл должен попасть по данным сервера: мультфильм → Cartoons, tv-шоу из Movies → Series.
function desiredType(it) {
  if (it.animation === true) return "cartoon";
  if (String(it.catalogId || "").startsWith("tv_") && it.type === "movie") return "series";
  return it.type || "movie";
}
const isSeriesLike = (it) => String(it.catalogId || "").startsWith("tv_") ||
  it.season != null || it.episode != null || !!it.seriesDir;
