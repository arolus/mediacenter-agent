...
const {
  collection, doc, getDocs, onSnapshot, query, where, updateDoc, setDoc, deleteDoc, serverTimestamp
...
async function normalizeCollection(ctx) {
  ...
  const seriesLike = (it) => {
    ...
    const n = groupCount.get(groupKey(it)) || 0;
    // tv_ от TMDb — сериал, но одинокий файл «сериала» — почти наверняка ошибка распознавания
    // (мультфильм-сиквел, совпавший с сериалом по названию/году) — не трогаем его как сериал.
    if (cat.startsWith("tv_")) return n >= 2;
  ...
  for (const it of all) {
    ...
    try {
      ...
      renamed++;
      // Оригинальное имя файла НЕ теряется при переименовании: оставляем заготовку дока
      // нового пути с originalName — рескан (syncLibrary) её подхватит и сохранит.
      await setDoc(doc(db, "devices", config.device.id, "library", libIdFor(target)),
        { originalName: it.originalName || it.fileName || path.basename(it.filePath) }, { merge: true });
