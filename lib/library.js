...
async function syncLibrary(ctx) {
  ...
  for (const [type, dir] of Object.entries(dirs)) {
    ...
    total += files.length;
    // Считаем файлы в подпапках первого уровня: папка = сериал, только если в ней ≥2 файлов.
    // Одиночный фильм в папке-обёртке («Шрэк (2004)/Шрэк 2.mkv») — не сериал.
    const dirCounts = new Map();
    for (const f of files) {
      const seg = path.relative(dir, f.filePath).split(path.sep);
      if (seg.length > 1) dirCounts.set(seg[0], (dirCounts.get(seg[0]) || 0) + 1);
    }
    for (const f of files) {
      ...
      const seriesDir = segs.length > 1 && (dirCounts.get(segs[0]) || 0) >= 2 ? segs[0] : null;
      ...
      if (type === "series" || type === "cartoon") {
        // Серия без SxxExx: хвостовой номер в имени («…классики..151.avi» → серия 151, название без
        // хвоста). ТОЛЬКО у сериалов или внутри папки сериала: плоский файл в Cartoons с номером
        // («Шрэк 2») — это сиквел, а не серия.
        if (episode == null && (seriesDir || type === "series")) {
