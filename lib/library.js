...
const TMDB_FIELDS = ["tmdbId", "catalogId", "poster", "backdrop", "backdrops", "overview", "cast", "rating", "animation", "collection",
  "castX", "directorsX", "genres", "director", "writers", "country", "studio", "premiered", "votes", "tagline", "runtime", "trailer", "budget", "revenue", "imdbId", "imdbRating", "imdbVotes", "enrichV"];
  ...
  for (const [type, dir] of Object.entries(dirs)) {
    ...
    for (const f of files) {
      ...
      if (prev && prev.fileName === f.fileName && prev.type === type && (prev.seriesDir ?? null) === seriesDir) {
        ...
        // бэкофилл: дата добавления (mtime файла) — для сортировки «новые сверху»
        if (!prev.addedAt && f.mtimeMs) await setDoc(doc(libCol, id), { addedAt: f.mtimeMs }, { merge: true });
        continue;
      }
      ...
      await setDoc(doc(libCol, id), {
        ...
        // дата добавления: сохраняется при рескане, у новых — mtime файла
        addedAt: (prev && prev.addedAt) || f.mtimeMs || Date.now(),
