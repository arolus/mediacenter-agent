...
function parseName(fileName) {
  // parse-torrent-title не срезает некоторые расширения (напр. .m4v) — убираем реальное сами,
  // иначе оно попадает в название и ломает поиск в TMDb ("Dead Poets Society.m4v").
  const clean = String(fileName).replace(/\.(mkv|mp4|avi|mov|wmv|m4v|mpg|mpeg|mpe|mpv|ts|m2ts|3gp|ogv|vob|flv|webm)$/i, "");
  const p = ptt.parse(clean);
...
module.exports = { parseName, enrich, hasTmdb, tmdbGet };
