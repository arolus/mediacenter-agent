...
const { currentSha } = require("./updater");
...
function startLocalServer(ctx) {
  ...
  // Режим (детский/обычный) у каждого устройства свой; общий config/tv.mode остаётся
  // запасным вариантом для нод, которым его ещё не задавали.
  let deviceMode = null;
  const unsubDev = onSnapshot(doc(db, "devices", config.device.id), (snap) => {
    const d = snap.data() || {};
    const name = d.name;
    const mode = d.mode === "kids" || d.mode === "normal" ? d.mode : null;
    if ((!name || name === deviceName) && mode === deviceMode) return;
  ...
  // Там же режим: "kids" — все замки (выход только по коду), "normal" — обычный телевизор.
  const PIN_CACHE = path.join(__dirname, "..", "cache", "kiosk.json");
  let kioskPin = "";
  let kioskMode = "normal";
  try {
    const c = JSON.parse(fs.readFileSync(PIN_CACHE, "utf8"));
    kioskPin = String(c.pin || "");
    kioskMode = c.mode === "kids" ? "kids" : "normal";
  } catch (_) {}
  const unsubKiosk = onSnapshot(doc(db, "config", "tv"), (snap) => {
    ...
    kioskMode = snap.data()?.mode === "kids" ? "kids" : "normal";
    try {
      fs.mkdirSync(path.dirname(PIN_CACHE), { recursive: true });
      fs.writeFileSync(PIN_CACHE, JSON.stringify({ pin: kioskPin, mode: kioskMode, at: Date.now() }));
    } catch (e) { console.error("киоск: не сохранил код:", e.message); }
    // Режим переключают из дашборда — TV-страница должна узнать об этом сразу
    for (const c of sseClients) { try { c.write("data: change\n\n"); } catch (_) {} }
  }, () => {});
  ...
  const server = http.createServer((req, res) => {
    ...
    // version — не для показа, а чтобы страница узнала, что агент перезапустился на новом коде,
    // и перезагрузила себя: TV живёт неделями открытым, и после апдейта показывала старый UI.
    if (u.pathname === "/api/device")
      return sendJson(res, { name: deviceName, id: config.device.id, version: (currentSha() || "dev").slice(0, 7) });
    ...
    if (u.pathname === "/api/kiosk") return sendJson(res, { pinSet: !!kioskPin, mode: deviceMode || kioskMode });
    ...
    if (u.pathname === "/api/play") return play(ctx, find(u.searchParams.get("id")), port, res,
      u.searchParams.get("via") === "app", u.searchParams.get("fromStart") === "1");
    ...
    if (u.pathname === "/api/movie") return movieInfo(ctx, u, res);
...
function play(ctx, item, port, res, viaApp, fromStart) {
  const { db, config } = ctx;
  ...
  // У VLC интент принимает «прихожая» StartActivity и наши extra теряет (мусорное название
  // из метаданных файла, игнор «начать сначала») — целимся прямо в плеерную активность.
  const target = pkg.startsWith("org.videolan.vlc")
    ? ["-n", `${pkg}/org.videolan.vlc.gui.video.VideoPlayerActivity`]
    : ["-p", pkg];
  const args = ["start", "-a", "android.intent.action.VIEW", "-d", url, "-t", "video/*", ...target,
  ...
  // «Начать сначала»: extra from_start VLC игнорирует, а position (мс) отрабатывает — им и
  // отматываем в ноль, иначе плеер молча продолжит с сохранённой секунды.
  if (fromStart) args.push("--ez", "from_start", "true", "--el", "position", "0");
  if (viaApp) {
...
// Карточка ещё не скачанного фильма (TV открывает её из фильмографии и коллекций).
// Данные даёт Cloud Function `movie`; на диске кэшируем навсегда — они не меняются.
const MOVIE_DIR = path.join(__dirname, "..", "cache", "movies");
function movieFnUrl(config, tmdbId, kind) {
  const pid = config.firebase && config.firebase.projectId;
  const q = `?kind=${encodeURIComponent(kind)}&tmdbId=${encodeURIComponent(tmdbId)}`;
  if (config.emulator) {
    const host = (config.emulator.firestore && config.emulator.firestore.host) || "127.0.0.1";
    return `http://${host}:5001/${pid}/us-central1/movie${q}`;
  }
  return `https://us-central1-${pid}.cloudfunctions.net/movie${q}`;
}
function movieInfo(ctx, u, res) {
  const tmdbId = String(u.searchParams.get("tmdbId") || "").trim();
  const kind = u.searchParams.get("kind") === "tv" ? "tv" : "movie";
  if (!tmdbId) return sendJson(res, { error: "нет tmdbId" }, 400);
  const file = path.join(MOVIE_DIR, `${kind}_${tmdbId.replace(/[^0-9]/g, "")}.json`);
  if (fs.existsSync(file)) {
    res.writeHead(200, { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-cache" });
    return fs.createReadStream(file).on("error", () => res.end()).pipe(res);
  }
  (async () => {
    const r = await fetch(movieFnUrl(ctx.config, tmdbId, kind));
    const body = await r.text();
    if (!r.ok) { res.writeHead(r.status, { "Content-Type": "application/json; charset=utf-8" }); return res.end(body); }
    try { fs.mkdirSync(MOVIE_DIR, { recursive: true }); fs.writeFileSync(file, body); } catch (_) {}
    res.writeHead(200, { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-cache" });
    res.end(body);
...
function personInfo(ctx, u, res) {
  ...
  // «v3|» в хэше = инвалидация дискового кэша при смене формата ответа функции (v3: + votes)
  const file = path.join(PERSON_DIR, crypto.createHash("sha1").update("v3|" + name).digest("hex").slice(0, 20) + ".json");
