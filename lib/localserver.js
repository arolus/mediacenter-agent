...
const posSeen = new Map();     // libId -> { offset, at }
function notePlaybackOffset(ctx, item, offset, size) {
  if (!item || !size || !item.runtime) return;
  const prev = posSeen.get(item.id) || { offset: 0, at: 0 };
  const now = Date.now();
  if (offset <= prev.offset || now - prev.at < 15000) {
    if (offset > prev.offset) posSeen.set(item.id, { offset, at: prev.at });
    return;
  }
  posSeen.set(item.id, { offset, at: now });
  const durMs = item.runtime * 60000;
  const posMs = Math.round((offset / size) * durMs);
  const TAIL_MS = 20 * 60000;                    // до конца меньше 20 минут — считаем просмотренным
