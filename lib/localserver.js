...
const { tmdbGet, hasTmdb } = require("./recognizer");
...
// Полная фильмография персоны для страницы актёра/режиссёра. Агент дёргает TMDb сам
// (ключ у него уже есть — используется при добавлении с торрента); кэш в cache/persons/.
...
function personInfo(ctx, u, res) {
  ...
  if (!hasTmdb(ctx.config)) return sendJson(res, { error: "на устройстве нет TMDb-ключа" }, 501);
  ...
  (async () => {
    const found = await tmdbGet(`search/person?include_adult=false&query=${encodeURIComponent(name)}`, ctx.config);
    const top = (found.results || [])[0];
    if (!top) return sendJson(res, { error: "не найдено в TMDb" }, 404);
    const d = await tmdbGet(`person/${top.id}?append_to_response=combined_credits`, ctx.config);
    // биография на ru часто пустая — добираем английскую
    let bio = d.biography || "";
    if (!bio) {
      try { bio = (await tmdbGet(`person/${top.id}?language=en-US`, ctx.config)).biography || ""; } catch (_) {}
    }
    const seen = new Map();
    const all = [...(d.combined_credits?.cast || []), ...(d.combined_credits?.crew || [])];
    for (const c of all) {
      if (c.media_type !== "movie" && c.media_type !== "tv") continue;
      // из crew берём только постановочные роли — не «продюсер 150 шоу»
      if (c.job && !["Director", "Screenplay", "Writer", "Story"].includes(c.job)) continue;
      const key = c.media_type + "_" + c.id;
      let e = seen.get(key);
      if (!e) {
        const date = c.media_type === "tv" ? c.first_air_date : c.release_date;
        e = { tmdbId: c.id, kind: c.media_type,
              title: (c.media_type === "tv" ? c.name : c.title) || "",
              year: date ? Number(date.slice(0, 4)) : null,
              poster: c.poster_path || null, rating: c.vote_average || 0, roles: [] };
        seen.set(key, e);
      }
      const role = c.character || (c.job === "Director" ? "режиссёр" : c.job ? "сценарий" : "");
      if (role && !e.roles.includes(role)) e.roles.push(role);
    }
    const out = {
      name: d.name || name, biography: bio,
      birthday: d.birthday || null, deathday: d.deathday || null,
      place: d.place_of_birth || "", photo: d.profile_path || null,
      credits: [...seen.values()]
    };
    try { fs.mkdirSync(PERSON_DIR, { recursive: true }); fs.writeFileSync(file, JSON.stringify(out)); } catch (_) {}
    sendJson(res, out);
