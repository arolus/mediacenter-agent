...
function startLocalServer(ctx) {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (u.pathname === "/api/images") return images(config, find(u.searchParams.get("id")), res);
...
// Скриншоты (кадры) фильма/сериала с TMDb.
async function images(config, item, res) {
  if (!item || !item.tmdbId) return sendJson(res, { backdrops: [] });
  const kind = item.catalogId ? String(item.catalogId).split("_")[0]
    : (item.type === "series" || item.season != null ? "tv" : "movie");
  try {
    let url = `https://api.themoviedb.org/3/${kind}/${item.tmdbId}/images`;
    const opts = {};
    if (config.tmdbBearer) opts.headers = { Authorization: "Bearer " + config.tmdbBearer };
    else url += `?api_key=${config.tmdbApiKey}`;
    const r = await fetch(url, opts);
    if (!r.ok) return sendJson(res, { backdrops: [] });
    const d = await r.json();
    sendJson(res, { backdrops: (d.backdrops || []).slice(0, 8).map((b) => b.file_path) });
  } catch (_) { sendJson(res, { backdrops: [] }); }
}
