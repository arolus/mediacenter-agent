...
function startLocalServer(ctx) {
  ...
  // Закрепляем альбомную ориентацию системно (переживает запуск/выход VLC).
  // Требует разового гранта: adb shell appops set com.termux WRITE_SETTINGS allow
  execFile("settings", ["put", "system", "accelerometer_rotation", "0"], { timeout: 5000 }, (e1) => {
    if (e1) return; // не Android или нет прав — тихо пропускаем
    execFile("settings", ["put", "system", "user_rotation", "1"], { timeout: 5000 }, (e2) => {
      if (!e2) console.log("✓ системная ориентация закреплена: ландшафт");
    });
  });
