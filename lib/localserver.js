...
const posSeen = new Map();     // libId -> { at, offset }
// Плееры при открытии дёргают ХВОСТ файла (в MKV там индекс) — такой запрос не означает,
// что фильм досмотрен: его отбрасываем. Позиция — это последнее смещение обычного чтения,
// плеер идёт по файлу последовательно, а при перемотке просто прыгает.
const META_TAIL = 0.98;
function notePlaybackOffset(ctx, item, offset, size) {
  if (!item || !size || !item.runtime) return;
  if (offset / size > META_TAIL) return;                  // чтение метаданных, не просмотр
  const prev = posSeen.get(item.id) || { at: 0 };
  const now = Date.now();
  if (now - prev.at < 15000) return;                      // не чаще раза в 15с — это запись в Firestore
  posSeen.set(item.id, { at: now, offset });
