...
function startLocalServer(ctx) {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (u.pathname === "/api/position") return setPosition(ctx, find(u.searchParams.get("id")), u, res);
    ...
    if (u.pathname === "/stream" || u.pathname.startsWith("/stream/")) {
      ...
      return stream(find(id), config, req, res, ctx);
    }
...
function libraryView(library) {
  return library.map((it) => ({
    ...
    // где остановились (мс) — VLC возвращает позицию при выходе, отсюда же её берёт
    // лента «Продолжить просмотр» на домашнем экране Android TV
...
const posSeen = new Map();     // libId -> { at, offset, streak }
// Позицию берём из Range-запросов плеера, но осторожно: при открытии файла VLC читает хвост
// (в MKV там индекс) и прыгает по нему seek'ами. Одиночный скачок — это не «досмотрели»:
// засчитываем только последовательное чтение, когда следующий запрос идёт рядом с предыдущим.
const POS_WRITE_MS = 8000;     // чаще писать в Firestore незачем
const NEAR = 0.05;             // «рядом» — в пределах 5% файла
function notePlaybackOffset(ctx, item, offset, size) {
  if (!item || !size || !item.runtime) return;
  const prev = posSeen.get(item.id) || { at: 0, offset: -1, streak: 0 };
  const near = prev.offset >= 0 && Math.abs(offset - prev.offset) / size < NEAR;
  const streak = near ? prev.streak + 1 : 1;   // скачок (seek/метаданные) начинает счёт заново
  const now = Date.now();
  if (now - prev.at < POS_WRITE_MS) { posSeen.set(item.id, { at: prev.at, offset, streak }); return; }
  posSeen.set(item.id, { at: now, offset, streak });
  if (streak < 2) return;                      // одиночному запросу не верим

  const durMs = item.runtime * 60000;
  const posMs = Math.round((offset / size) * durMs);
  const TAIL_MS = 20 * 60000;                  // ближе 20 минут к концу — считаем просмотренным
  const finished = durMs - posMs <= TAIL_MS;
  updateDoc(doc(ctx.db, "devices", ctx.config.device.id, "library", item.id),
    finished ? { position: 0, watched: true, watchedAt: serverTimestamp() } : { position: posMs })
    .catch(() => {});
}

// Отдача файла с поддержкой Range (для перемотки в плеере).
function stream(item, config, req, res, ctx) {
  ...
  if (range) {
    ...
    res.statusCode = 206;
    if (ctx) notePlaybackOffset(ctx, item, start, stat.size);   // отсюда знаем, где смотрят
...
// Позиция остановки: TV-приложение присылает её после выхода из VLC (тот отдаёт extra_position).
// Досмотренное до конца (или почти) считаем просмотренным и позицию обнуляем — иначе фильм
// вечно висел бы в «Продолжить просмотр» на последней секунде.
function setPosition(ctx, item, u, res) {
  const { db, config } = ctx;
  if (!item) return sendJson(res, { error: "не найдено" }, 404);
  const pos = Math.max(0, parseInt(u.searchParams.get("pos") || "0", 10) || 0);
  const durMs = (item.runtime || 0) * 60000;
  const TAIL_MS = 20 * 60000;   // хвост фильма: досмотрел, если ближе этого к концу
  const finished = durMs > 0 && pos > 0 && durMs - pos <= TAIL_MS;
  const patch = finished
    ? { position: 0, watched: true, watchedAt: serverTimestamp() }
    : { position: pos };
  updateDoc(doc(db, "devices", config.device.id, "library", item.id), patch)
    .then(() => sendJson(res, { ok: true, position: finished ? 0 : pos, finished }))
    .catch((e) => sendJson(res, { error: e.message }, 500));
}
...
function homeScreenView(library, lanIp, port) {
  ...
  const items = library.map((it) => ({
    ...
    positionMs: it.position || 0,
    watched: it.watched === true,
    addedAt: it.addedAt || 0
  }));
  return {
    // «Продолжить просмотр» — только начатое и недосмотренное, свежее сверху
    continueWatching: items.filter((x) => x.positionMs > 60000 && !x.watched)
