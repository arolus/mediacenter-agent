...
// Возвращает [{ filePath, fileName, sizeBytes, mtimeMs }]
...
function walk(dir, re, out) {
  ...
  for (const ent of entries) {
    ...
    if (ent.isDirectory()) {
      ...
      // mtime = «дата добавления» на устройство (сортировка «новые сверху» на TV)
      try { const st = fs.statSync(full); size = st.size; mtimeMs = Math.round(st.mtimeMs); } catch (_) {}
      out.push({ filePath: full, fileName: ent.name, sizeBytes: size, mtimeMs });
    }
  }
}
