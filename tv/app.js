...
async function load() {
  ...
  try {
    ...
    // focusId не восстанавливаем сознательно: он прокручивал сетку к последней карточке,
    // и после каждого обновления интерфейса список «открывался в середине». Возвращаем сам
    // экран, но сетку — с верха.
    if (saved && saved.screen && saved.screen !== "categories" &&
        Date.now() - (saved.at || 0) < 60000 &&
        !new URLSearchParams(location.search).get("play")) navigate({ ...saved, focusId: null });
  } catch (_) {}
...
// 1 серия, 2 серии, 5 серий — русское склонение по числу.
        ...
        <div class="flex min-w-0 flex-1 flex-col">
          ...
          <div class="mt-1 flex min-h-0 ${multi ? "flex-1" : ""} space-x-6">
            ${multi ? `<div class="flex min-w-0 flex-1 flex-col">
              <div id="detail-desc" tabindex="0" class="thin-scroll min-h-0 min-w-0 flex-1 overflow-y-auto rounded-md pr-1 text-[clamp(12px,calc(var(--uivh)*2.4),16px)] leading-snug text-zinc-200 outline-none drop-shadow focus:ring-2 focus:ring-violet-500/40">${esc(i.overview || "Нет описания")}</div>
              <div class="mt-auto flex-none space-y-1 pt-3">${metaTable}</div>
