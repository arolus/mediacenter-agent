...
function renderDetail() {
  ...
  app.innerHTML = `
    <div class="relative h-full overflow-hidden">
      ...
      <div class="mb-[clamp(16px,calc(var(--uivh)*4),36px)] flex-none truncate text-center text-[clamp(24px,calc(var(--uivh)*5.4),44px)] font-extrabold leading-tight tracking-tight drop-shadow-lg">${esc(i.title)}${i.year ? ` <span class="font-semibold text-zinc-400">(${i.year})</span>` : ""}${multi ? ` <span class="text-[0.55em] font-semibold text-zinc-400">· ${eps.length} ${plural(eps.length, "серия", "серии", "серий")}</span>` : ""}</div>
